...
# --- FIX FOR NO-LOSS UPDATE ---
# 1. Use persistent disk if Render has it (/data), else fallback
DB_PATHS = ["/data/zondi_db.json", "zondi_db.json"] # /data survives deploys if you add Disk on Render
UPLOAD_DIR = "/data/uploads" if os.path.exists("/data") else "uploads"
os.makedirs(UPLOAD_DIR, exist_ok=True)

# Try find existing DB
DB_FILE = next((p for p in DB_PATHS if os.path.exists(p)), DB_PATHS[0])

# Supabase / Postgres upgrade ready (optional - add DATABASE_URL in Render env)
USE_SUPABASE = False
...
try:
    ...
    if db_url and "supabase" in db_url.lower():
        ...
        USE_SUPABASE = True
        print("USING SUPABASE - NO MORE CREDENTIAL LOSS")
except Exception as e:
    print(f"Using local JSON, add Supabase for 100% persistence: {e}")

if not os.path.exists(DB_FILE):
    with open(DB_FILE,"w") as f: json.dump({"locations":[],"sos_events":[],"users":[],"purchases":[],"videos":[],"app_version":"2.0"}, f)

def load_db():
    if USE_SUPABASE:
        # For now keep JSON as cache, Supabase will be full migration next
        try:
...
def save_db(db):
    db["app_version"] = "2.0" # for silent update detection
    db["last_update"] = datetime.datetime.now().isoformat()
    with open(DB_FILE,"w") as f: json.dump(db,f,indent=2)
    # If Supabase enabled, also sync users there (so they never lose)
    if USE_SUPABASE and supabase_client:
        try:
            # Don't block if fails
            pass
        except: pass
...
<div style="padding:12px 16px;display:flex;justify-content:space-between;align-items:center;border-bottom:1px solid #eee;background:#fff;position:sticky;top:0;z-index:20"><div style="font-size:10px;color:#888">GA-RANKUWA • SOSHANGUVE • MABOPANE</div><div style="display:flex;gap:10px;font-size:12px"><a href="/" style="text-decoration:none;color:#111">Home</a><a href="/shop" style="text-decoration:none;color:#111">Shop</a><a href="/login" style="text-decoration:none;color:#fff;background:#111;padding:6px 14px;border-radius:20px">Login</a><span id="ver" style="font-size:9px;color:#16a34a;background:#f0fdf4;padding:3px 6px;border-radius:6px">v2.0 LIVE</span></div></div>"""

HOME_HTML = NAV + """<div style="max-width:420px;margin:0 auto;min-height:100vh;display:flex;flex-direction:column;justify-content:center;align-items:center;padding:24px;text-align:center"><h1 style="font-size:28px;font-weight:800">Security that feels like family<br><span style="color:#2563eb">protects like SAPS.</span></h1><p style="color:#666;font-size:13px">📍 pins + 🎥 5min T780 + WhatsApp • v2.0 silent update</p><a href="/login" style="background:#111;color:#fff;padding:14px 28px;border-radius:12px;text-decoration:none;font-weight:700;width:280px;text-align:center">Enter App</a></div>"""
...
// --- SILENT UPDATE FIX: Auto-login if already have auth ---
window.onload=()=>{
 let a=JSON.parse(localStorage.getItem('zondi_auth')||'null');
 if(a && a.email){
   document.getElementById('autoMsg').innerText='Welcome back '+a.email+' • checking updates...';
   fetch('/api/me?email='+a.email).then(r=>r.json()).then(d=>{
     if(d.ok){location.href=d.redirect}
   });
 }
 // Check for new version silently
 fetch('/api/version').then(r=>r.json()).then(v=>{
   if(v.version && localStorage.getItem('zondi_version') && localStorage.getItem('zondi_version')!=v.version){
     console.log('New features available: '+v.version);
   }
...
def version():
    return jsonify({"version":"2.0","message":"Silent features update","must_update":False})

@app.route("/api/me")
def me():
    email=request.args.get("email"); db=load_db()
    user=next((u for u in db["users"] if u.get("email")==email), None)
    if not user: return jsonify({"ok":False})
    return jsonify({"ok":True,"redirect":"/patrollers" if user.get("role")=="patroller" else "/clients","user":user})
...
def upload_video():
    ...
    f.save(fpath)
    db=load_db()
    db["videos"].append({"file": fname,"phoneId": phoneId,"address": request.form.get('address',''),"shop": request.form.get('shop',''),"name": request.form.get('name',''),"time": datetime.datetime.now().isoformat(),"lat": request.form.get('lat'),"lng": request.form.get('lng')})
